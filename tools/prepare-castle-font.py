...
Usage: uv run --with fonttools python tools/prepare-castle-font.py <variable.ttf>
...
text = ''.join(p.read_text(encoding='utf-8-sig') for p in (root/'native/app/castle').glob('*') if p.suffix in ('.cpp', '.h'))
chars = ''.join(sorted({c for c in text if ord(c) > 127 and c != '\ufeff'}))
assets = root/'app/src/main/assets/fonts'
(assets/'castle-glyphs.txt').write_text(chars, encoding='utf-8')
