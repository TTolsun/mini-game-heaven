...
4. BACK은 통로 편집 닫기, 합성창 닫기·배치 취소, 방어 일시정지, 준비 화면에서 종료의 순서로 처리합니다.
...
준비 단계에서만 통로와 왕좌를 바꿉니다. 침공은 Scene의 표시와 Model의 공격 판정에서 동일한 최단 경로를 사용하며, 경로가 없으면 시작할 수 없습니다. 통로 편집은 무료이므로 저장된 단절 상태를 자원 없이 복구할 수 있어야 합니다. 저장 경로는 castle-v1.txt를 유지하고 CASTLE 1을 읽되 새 저장은 CASTLE 2로 기록합니다. 근거: `native/app/castle/CastleModel.cpp`, `native/tests/CastleModelTests.cpp`, [D-011](decisions.md).
