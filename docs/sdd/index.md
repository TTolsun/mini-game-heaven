...
"코드로 확인한 구조" 아래 문서는 compile DB(arm64-v8a Debug, CMake 구성)를 기준으로 정적 분석한 사실과 이를 코드에 대조한 설명을 담습니다. 기기에서 실행해 본 결과가 아닙니다. "사람이 기록한 설계" 아래 문서는 파이프라인이 덮어쓰지 않으며, 설계 결정과 기기 관찰을 사람이 씁니다.
