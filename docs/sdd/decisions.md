...
## D-011 성 운영을 중심으로 확장하고 침공 동선을 편집한다

2026-09-19 사용자가 게임 콘셉트를 카이로소프트 「마왕성 스토리」로 명시했습니다. D-007의 수집·합성 중심 범위를 성 건설·모집·육성·침입 대응·탐험·성 등급 성장으로 확장합니다. 첫 단계에서는 기존 12칸의 안정적인 방 ID를 유지하고 상하좌우 통로와 왕좌 위치를 편집하도록 합니다. 몬스터는 배치된 방을 지키며 자유 이동·탐험·성 등급은 후속 구현입니다.

용사는 균일 비용의 방 그래프에서 BFS로 찾은 최단 경로를 따릅니다. 같은 거리에서는 낮은 방 번호부터 탐색합니다. 경로는 방을 반복 방문하지 않아 순환 통로의 함정 반복 공격을 방지합니다. 지도와 전투가 같은 경로를 참조하며 단절된 경로는 침공 전에 차단합니다. 편집과 왕좌 이동은 무료이고, 열린 방과 인접한 칸을 확장하면 가장 낮은 번호의 열린 이웃 하나에 연결합니다. 확장만으로 왕좌는 옮기지 않습니다. 기존의 공간 인접 시설 보너스는 유지합니다.

CASTLE 2는 통로와 왕좌를 추가하며 CASTLE 1은 순차 통로로 이전합니다. 통로가 끊긴 준비 상태도 저장하되 모든 열린 방은 공간상 연결돼 있어 무료 편집으로 복구할 수 있어야 합니다. 이전 앱에 새 저장 파일을 읽히는 역방향 이전은 지원하지 않습니다. 근거: CastleModel의 경로·저장 구현, CastleScene의 편집·경로 표시, CastleModelTests와 CastleSceneTests의 회귀 검증입니다.
